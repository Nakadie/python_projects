...
def load_words(file_name):
    ...
    # inFile: file
    in_file = open('D:\python_projects\intro to css\ProblemSet5\words.txt', 'r')
    # line: string
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        shift = 1
        while True:
            # build the shift dictonary
            code_dict = self.build_shift_dict(shift)
            # apply the shift
            test = self.apply_shift(self.code)
            # break the string into peices
            test = test.split(' ')
            # check if valid word
            points = 0
            for word in test:
                if is_word(word) == True:
                    points += 1
            # check number of valid words
            if points == len(test):
                answer = ' '.join(test)
                return(answer, 26-shift)
            else:
                shift += 1

cool = CiphertextMessage('jgnnq')
cool.decrypt_message()